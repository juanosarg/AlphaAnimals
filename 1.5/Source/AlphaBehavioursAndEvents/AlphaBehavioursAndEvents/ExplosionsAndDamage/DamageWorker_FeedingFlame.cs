    ...
    public class DamageWorker_FeedingFlame : DamageWorker_AddInjury
    {
        public override DamageWorker.DamageResult Apply(DamageInfo dinfo, Thing victim)
        {
            ...
            if (victim.Destroyed && map != null && pawn == null)
            {
                ...
                if (plant != null && victim.def.plant.IsTree && plant.LifeStage != PlantLifeStage.Sowing && victim.def != InternalDefOf.BurnedTree)
                {
                    ((DeadPlant)GenSpawn.Spawn(InternalDefOf.BurnedTree, victim.Position, map, WipeMode.Vanish)).Growth = plant.Growth;
                }
            }
