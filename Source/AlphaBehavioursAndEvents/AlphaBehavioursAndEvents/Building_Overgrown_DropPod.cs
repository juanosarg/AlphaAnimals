...
{
    public class Building_Overgrown_DropPod : ThingWithComps
    {
                        ...
                        RegionTraverser.BreadthFirstTraverse(this.GetRegion(RegionType.Set_Passable), (Region from, Region to) => true, delegate (Region r)
                        {
                            ...
                            for (int i = 0; i < list.Count; i++)
                            {
                                if (list[i] != this)
                                {
                                    ...
                                    {
                                        foundPawn = ((Building_Overgrown_DropPod)list[i]).spawnedPawns.Find(hasDefendHiveLord);
                                        if (foundPawn != null)
                                        {
                                            return true;
                                        }
        ...
        {
            Building_Overgrown_DropPod.spawnablePawnKinds.Clear();
            Building_Overgrown_DropPod.spawnablePawnKinds.Add(PawnKindDefOf.Megascarab);
            Building_Overgrown_DropPod.spawnablePawnKinds.Add(PawnKindDefOf.Spelopede);
            Building_Overgrown_DropPod.spawnablePawnKinds.Add(PawnKindDefOf.Megaspider);
        }
        ...
        public override void DeSpawn(DestroyMode mode = DestroyMode.Vanish)
        {
            ...
            {
                lords[i].ReceiveMemo(Building_Overgrown_DropPod.MemoDeSpawned);
            }
        }
        ...
        public override void PostApplyDamage(DamageInfo dinfo, float totalDamageDealt)
        {
            if (dinfo.Def.ExternalViolenceFor(this) && dinfo.Instigator != null && dinfo.Instigator.Faction != null)
            {
                ...
                if (lord != null)
                {
                    lord.ReceiveMemo(Building_Overgrown_DropPod.MemoAttackedByEnemy);
                }
            }
            if (dinfo.Def == DamageDefOf.Flame && (float)this.HitPoints < (float)base.MaxHitPoints * 0.3f)
            {
                ...
                if (lord2 != null)
                {
                    lord2.ReceiveMemo(Building_Overgrown_DropPod.MemoBurnedBadly);
                }
            }
        ...
        public override void Kill(DamageInfo? dinfo = null, Hediff exactCulprit = null)
        {
            if (base.Spawned && (!dinfo.HasValue || dinfo.Value.Category != DamageInfo.SourceCategory.Collapse))
            {
                ...
                for (int i = 0; i < lords.Count; i++)
                {
                    lords[i].ReceiveMemo(Building_Overgrown_DropPod.MemoDestroyedNonRoofCollapse);
                }
            }
        ...
        private void CalculateNextPawnSpawnTick()
        {
            ...
            this.nextPawnSpawnTick = Find.TickManager.TicksGame + (int)(Building_Overgrown_DropPod.PawnSpawnIntervalDays.RandomInRange * 60000f / (num * Find.Storyteller.difficulty.enemyReproductionRateFactor));
        }
        ...
        private bool TrySpawnPawn(out Pawn pawn)
        {
            ...
            IEnumerable<PawnKindDef> source = from x in Building_Overgrown_DropPod.spawnablePawnKinds
